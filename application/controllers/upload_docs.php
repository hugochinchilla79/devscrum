...
class Upload_docs extends CI_Controller {
	...
	function index(){
		if(!$this->session->userdata('login_status')==1) {
			...
			$data['param'] = isset($_GET['v'])? $_GET['v']:"";
			$data['dir'] = directory_map("uploads/".$data['param']);
			$data['ruta'] = "uploads/".$data['param'];
			$this->load->view('templates/header',$this->data);
			$this->load->view('templates/navside',$this->data);
			$this->load->view("Subir/subida_docs",$data);;
	}	
	...
	/*function crear_directorio(){
		...
		if(mkdir(getcwd().$ruta_nuevo)){
			$data['prueba'] = "Directorio creado exitosamente";
		}else{
			$data['prueba'] = "Error en la creacion del directorio";
		}
		$this->load->view('templates/header',$this->data);
		$this->load->view('templates/navside',$this->data);
		$this->load->view("Subir/subida_docs",$data);
	...
	function eliminar_directorio(){
		...
		if (rmdir(getcwd().$ruta_eliminar)) {
			$data['prueba'] = "Directorio eliminado exitosamente";
		}else{
			$data['prueba'] = "Error en la eliminacion del directorio";
		}
		$this->load->view('templates/header',$this->data);
		$this->load->view('templates/navside',$this->data);
		$this->load->view("Subir/subida_docs",$data);
	...
	function do_upload(){
		...
		if ($this->form_validation->run()==true) {
			...
			$config['allowed_types'] = "|docx|pdf|xlsx|pptx";
			...
			if (!$this->upload->do_upload()) {
				...
				$this->load->view('Subir/subida_docs',$error);
			}else{
				...
				$dif = pathinfo($imagen);
				$subir = $this->upload_model->subir_dif_archivos($titulo,$imagen);
				//$data['titulo'] = $titulo;
				$data['imagen'] = $imagen;
				$data['extension'] = $dif['extension'];
				$data['titulo'] = $this->upload->get_extension($file_info['file_path']);
				$this->load->view('templates/header',$this->data);
				$this->load->view('templates/navside',$this->data);
				$this->load->view('Subir/subida_completa_vista',$data);
			}
		}else{
